...
bool HttpRequest::UserVerify(const string &name, const string &pwd, bool isLogin) {
    ...
    bool flag = !isLogin;
    string order = "SELECT username, password FROM user WHERE username='" + name + "' LIMIT 1";
    ...
    if (res) {
        ...
        if ((row = mysql_fetch_row(res)) != nullptr) {
            ...
            if (isLogin) {
                flag = (pwd == password);
                if (!flag) LOG_DEBUG("Password error!");
            } else {
                flag = false;
    ...
    if (!isLogin && flag) {
        order = "INSERT INTO user(username, password) VALUES('" + name + "', '" + pwd + "')";
        LOG_DEBUG("%s", order.c_str());
        if (mysql_query(sql, order.c_str())) {
